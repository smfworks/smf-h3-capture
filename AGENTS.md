...
- Do not read or sync SPA packs (`localStorage` key `smf.h3-longform-capture.pack.v2`). Point the user at the iframe.
...
Sidebar **H3 Capture**, the right-of-chat pane, or ⌘K → **Open H3 Capture** / **Open H3 Capture pane**. Default embed is the live app. **Local** probes `http://127.0.0.1:5173/` (`npm run dev`) then `http://127.0.0.1:4173/` (`npm run preview`) in `h3-longform-capture/app`.
